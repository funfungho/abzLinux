...
- The computer’s processor (or CPU) works at an elemental level, executing programs in what is called *machine language*. This is a numeric code that describes extremely small operations, such as “add this byte,” “point to this location in memory,” or “copy this byte.” Each of these instructions is expressed in **binary** (ones and zeros)
- This problem was overcome by the advent of *assembly language*, which replaced the numeric codes with (slightly) easier-to-use character mnemonics such as `CPY` (for copy) and `MOV` (for move). Programs written in assembly language are processed into machine language by a program called an *assembler*
...
- A process often used in conjunction with compiling is called *linking*
    ...
    - A program called a *linker* is used to form the **connections** between the **output of the compiler** and the **libraries that the compiled program requires**. The final result of this process is the executable program file, ready for use
...
- The `.h` files are known as header files. Header files contain **descriptions** of the routines included in a source code file or library. For the compiler to connect the modules, it must receive a description of all the modules needed to complete the entire program
    - `#include "getopt.h"` instructs the compiler to read the file `getopt.h` (as the compiler reads the source code in `diction.c`) to **“know”** what’s in `getopt.c`
    ...
    - Other include statements
        ...
        - These also refer to header files, but they refer to header files that live outside the current source tree. They are supplied by the system to support the compilation of every program
